...
- [extensions/vscode-api-tests/src/singlefolder-tests/window.test.ts](extensions/vscode-api-tests/src/singlefolder-tests/window.test.ts)
...
- [src/vs/base/common/policy.ts](src/vs/base/common/policy.ts)
...
- [src/vs/platform/configuration/common/configurations.ts](src/vs/platform/configuration/common/configurations.ts)
...
- [src/vs/platform/configuration/test/common/policyConfiguration.test.ts](src/vs/platform/configuration/test/common/policyConfiguration.test.ts)
...
- [src/vs/platform/policy/common/filePolicyService.ts](src/vs/platform/policy/common/filePolicyService.ts)
- [src/vs/platform/policy/common/policy.ts](src/vs/platform/policy/common/policy.ts)
- [src/vs/platform/policy/common/policyIpc.ts](src/vs/platform/policy/common/policyIpc.ts)
- [src/vs/platform/policy/node/nativePolicyService.ts](src/vs/platform/policy/node/nativePolicyService.ts)
...
- [src/vs/workbench/api/browser/mainThreadEditorTabs.ts](src/vs/workbench/api/browser/mainThreadEditorTabs.ts)
...
- [src/vs/workbench/api/common/extHostDiagnostics.ts](src/vs/workbench/api/common/extHostDiagnostics.ts)
- [src/vs/workbench/api/common/extHostEditorTabs.ts](src/vs/workbench/api/common/extHostEditorTabs.ts)
...
- [src/vs/workbench/api/test/browser/extHostDiagnostics.test.ts](src/vs/workbench/api/test/browser/extHostDiagnostics.test.ts)
- [src/vs/workbench/api/test/browser/extHostEditorTabs.test.ts](src/vs/workbench/api/test/browser/extHostEditorTabs.test.ts)
- [src/vs/workbench/common/editor/binaryEditorModel.ts](src/vs/workbench/common/editor/binaryEditorModel.ts)
...
- [src/vs/workbench/contrib/editTelemetry/browser/editTelemetry.contribution.ts](src/vs/workbench/contrib/editTelemetry/browser/editTelemetry.contribution.ts)
- [src/vs/workbench/contrib/performance/electron-browser/performance.contribution.ts](src/vs/workbench/contrib/performance/electron-browser/performance.contribution.ts)
...
- [src/vs/workbench/contrib/preferences/browser/preferencesEditor.ts](src/vs/workbench/contrib/preferences/browser/preferencesEditor.ts)
...
- [src/vs/workbench/contrib/preferences/browser/preferencesWidgets.ts](src/vs/workbench/contrib/preferences/browser/preferencesWidgets.ts)
...
- [src/vs/workbench/contrib/preferences/browser/settingsLayout.ts](src/vs/workbench/contrib/preferences/browser/settingsLayout.ts)
...
- [src/vs/workbench/services/configuration/test/common/testServices.ts](src/vs/workbench/services/configuration/test/common/testServices.ts)
...
- [src/vs/workbench/services/preferences/test/browser/preferencesService.test.ts](src/vs/workbench/services/preferences/test/browser/preferencesService.test.ts)
- [src/vs/workbench/services/workingCopy/test/browser/workingCopyEditorService.test.ts](src/vs/workbench/services/workingCopy/test/browser/workingCopyEditorService.test.ts)
- [src/vs/workbench/test/browser/codeeditor.test.ts](src/vs/workbench/test/browser/codeeditor.test.ts)
...
The `IConfigurationRegistry` is the central point for registering settings. It manages `IConfigurationNode` objects which define properties, default values, and metadata like `ConfigurationScope` (Application, Machine, Window, Resource) [src/vs/platform/configuration/common/configurationRegistry.ts:34-127](). Settings are categorized by scope to determine where they can be stored (e.g., `APPLICATION_SCOPES`, `LOCAL_MACHINE_SCOPES`, `WORKSPACE_SCOPES`) [src/vs/workbench/services/configuration/common/configuration.ts:14-19]().

### Configuration Models
Settings are processed and stored in a hierarchy of `ConfigurationModel` instances [src/vs/platform/configuration/common/configurationModels.ts:14-15]():
*   **Default**: Aggregated from `IConfigurationRegistry`. Managed by `DefaultConfiguration`, which handles extension-contributed defaults and language-specific overrides [src/vs/workbench/services/configuration/browser/configuration.ts:32-60](). It caches default overrides in `localStorage` to speed up startup [src/vs/workbench/services/configuration/browser/configuration.ts:78-83]().
*   **Application**: Global settings that apply to all profiles, typically stored in `argv.json` [src/vs/workbench/services/configuration/browser/configuration.ts:129-132]().
*   **User**: Profile-specific global settings, managed by `UserConfiguration` [src/vs/workbench/services/configuration/browser/configurationService.ts:79]().
*   **Workspace**: Settings specific to the current `.code-workspace` file, managed by `WorkspaceConfiguration` [src/vs/workbench/services/configuration/browser/configurationService.ts:81]().
*   **Folder**: Settings in `.vscode/settings.json` within a workspace folder, managed by `FolderConfiguration` [src/vs/workbench/services/configuration/browser/configurationService.ts:82]().

### Data Flow: Configuration Merging
The `WorkspaceService` (implementing `IWorkbenchConfigurationService`) aggregates these models into a single `Configuration` object. It uses a `Barrier` to ensure remote user configurations are loaded before initialization completes [src/vs/workbench/services/configuration/browser/configurationService.ts:65-85]().
...
```
Sources: [src/vs/platform/configuration/common/configurationRegistry.ts:34-127](), [src/vs/workbench/services/configuration/browser/configurationService.ts:65-85](), [src/vs/workbench/services/configuration/browser/configuration.ts:32-60](), [src/vs/platform/configuration/common/configurationModels.ts:14-15]()
...
*   **SettingsTree**: A `WorkbenchObjectTree` that renders individual settings using specialized renderers [src/vs/workbench/contrib/preferences/browser/settingsTree.ts:49](). It uses a `CachedListVirtualDelegate` to calculate row heights for different setting types [src/vs/workbench/contrib/preferences/browser/settingsTree.ts:16]().
*   **SettingsTreeModels**: Classes like `SettingsTreeSettingElement` wrap raw `ISetting` data, providing logic for `isConfigured`, `isUntrusted`, and `isAgentsWindowReadOnly` [src/vs/workbench/contrib/preferences/browser/settingsTreeModels.ts:113-160]().

### Implementation Details
The UI uses `AbstractSettingRenderer` to handle different value types (boolean, string, enum, object) by instantiating widgets like `ListSettingWidget`, `ObjectSettingCheckboxWidget`, or `ExcludeSettingWidget` [src/vs/workbench/contrib/preferences/browser/settingsTree.ts:74]().
...
```
Sources: [src/vs/workbench/contrib/preferences/browser/settingsEditor2.ts:74](), [src/vs/workbench/contrib/preferences/browser/settingsTree.ts:49-74](), [src/vs/workbench/contrib/preferences/browser/settingsTreeModels.ts:113-160](), [src/vs/workbench/contrib/codeEditor/browser/suggestEnabledInput/suggestEnabledInput.ts:55]()
...
### Context Key Service
The `IContextKeyService` maintains a key-value store of the editor's state. Expressions are parsed from "when" clauses using a `Scanner` and `Parser` [src/vs/platform/contextkey/common/contextkey.ts:173-185](). The grammar supports operators like `==`, `!=`, `=~` (regex), `in`, and boolean logic (`&&`, `||`, `!`) [src/vs/platform/contextkey/common/contextkey.ts:98-126]().

### Keybinding Resolver
The `KeybindingResolver` matches incoming keyboard events against `ResolvedKeybindingItem`s. It filters by `ContextKeyExpression` and selects the command with the highest `KeybindingWeight` [src/vs/platform/keybinding/common/keybindingResolver.ts:38-40]().

### Keybindings Editor
The `KeybindingsEditor` uses a `WorkbenchTable` to display command IDs, keybindings, and "when" clauses [src/vs/workbench/contrib/preferences/browser/keybindingsEditor.ts:37-74](). It allows users to define new bindings via the `DefineKeybindingWidget` [src/vs/workbench/contrib/preferences/browser/keybindingWidgets.ts:25]().
...
```
Sources: [src/vs/platform/keybinding/common/keybindingResolver.ts:38-40](), [src/vs/workbench/services/keybinding/browser/keybindingService.ts:36-40](), [src/vs/platform/contextkey/common/contextkey.ts:71-81](), [src/vs/platform/contextkey/common/contextkey.ts:173-185]()
...
*   **PolicyConfiguration**: A specialized configuration model that aggregates settings enforced by the OS (e.g., Windows Group Policy). It overrides all other configuration targets [src/vs/workbench/services/configuration/browser/configurationService.ts:76](), [src/vs/platform/configuration/common/configurations.ts:16]().
*   **Restricted Settings**: Settings marked as `restricted: true` in the registry [src/vs/workbench/services/preferences/common/preferences.ts:89](). If the workspace is untrusted (via `IWorkspaceTrustManagementService`), these settings are ignored at the workspace/folder level [src/vs/workbench/contrib/preferences/browser/settingsTreeModels.ts:178]().
*   **Agents Window Constraints**: In the "Agents Window" (sessions layer), settings can be marked as read-only (via `isAgentsWindowReadOnly`) to ensure a consistent environment for AI agents [src/vs/workbench/contrib/preferences/browser/settingsTreeModels.ts:158-160]().

| Component | Implementation Class | Purpose |
| :--- | :--- | :--- |
| **Policy Service** | `IPolicyService` | Fetches values from the underlying OS policy store [src/vs/platform/policy/common/policy.ts:43](). |
| **Sync Enablement** | `IUserDataSyncEnablementService` | Controls which settings are allowed to sync across machines [src/vs/workbench/contrib/preferences/browser/settingsTree.ts:59](). |
| **Trust Management** | `IWorkspaceTrustManagementService` | Tracks if the current workspace is trusted by the user [src/vs/workbench/contrib/preferences/browser/settingsEditor2.ts:50](). |

Sources: [src/vs/platform/configuration/common/configurations.ts:16](), [src/vs/workbench/services/configuration/browser/configurationService.ts:76](), [src/vs/workbench/contrib/preferences/browser/settingsTreeModels.ts:158-160](), [src/vs/workbench/services/preferences/common/preferences.ts:89]()