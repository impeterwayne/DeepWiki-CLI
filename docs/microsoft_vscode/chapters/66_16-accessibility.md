...
The `AccessibleView` is a specialized editor widget that provides a screen-reader-friendly way to inspect content that might otherwise be difficult to navigate, such as terminal history, chat responses, or inline completions [src/vs/workbench/contrib/accessibility/browser/accessibleView.ts:71-102]().

*   **IAccessibleViewService**: The primary service used to show content in the accessible view [src/vs/workbench/contrib/accessibility/browser/accessibleView.ts:30-30]().
*   **AccessibleContentProvider**: An interface implemented by features (like Terminal or Chat) to provide text content and symbols to the view [src/vs/platform/accessibility/browser/accessibleView.ts:30-30]().
*   **Navigation**: Supports navigating between items (e.g., next/previous chat response) and "Go to Symbol" for structured content [src/vs/workbench/contrib/accessibility/browser/accessibleView.ts:77-79]().

### Accessibility Signals
Signals provide audio cues (sounds) and ARIA alerts to notify users of events without requiring visual focus. This is managed by the `IAccessibilitySignalService` [src/vs/platform/accessibilitySignal/browser/accessibilitySignalService.ts:20-22]().

*   **Sounds**: Cues for events like breakpoints being hit, task completion, or error markers [src/vs/platform/accessibilitySignal/browser/accessibilitySignalService.ts:130-134]().
*   **Announcements**: Textual descriptions sent to screen readers via ARIA live regions [src/vs/platform/accessibilitySignal/browser/accessibilitySignalService.ts:124-128]().
...
```
**Sources:** [src/vs/workbench/contrib/accessibility/browser/accessibleView.ts:103-115](), [src/vs/workbench/contrib/terminalContrib/accessibility/browser/terminalAccessibleBufferProvider.ts:17-19](), [src/vs/workbench/contrib/chat/browser/actions/chatAccessibilityHelp.ts:28-36](), [src/vs/platform/accessibilitySignal/browser/accessibilitySignalService.ts:127-128]()
...
The editor provides specific help for screen reader users, including toggling "Screen Reader Optimized Mode" and announcing cursor positions [src/vs/workbench/contrib/codeEditor/browser/accessibility/accessibility.ts:22-92]().
*   **Tab Focus Mode**: Allows users to toggle whether the `Tab` key inserts a character or moves focus [src/vs/editor/common/standaloneStrings.ts:24-25]().
*   **Accessibility Help**: A dedicated dialog providing context-specific instructions for the editor, registered via `EditorAccessibilityHelpContribution` [src/vs/workbench/contrib/accessibility/browser/accessibility.contribution.ts:13-13]().

### Terminal Accessibility
The terminal uses a `BufferContentTracker` to mirror terminal output into a format that the `TerminalAccessibleBufferProvider` can render for the `AccessibleView` [src/vs/workbench/contrib/terminalContrib/accessibility/browser/terminalAccessibleBufferProvider.ts:17-32]().
*   **TextArea Sync**: The `TextAreaSyncAddon` synchronizes the terminal's `promptInputModel` with the xterm.js textarea to ensure screen readers can track the cursor and input text accurately [src/vs/workbench/contrib/terminalContrib/accessibility/browser/textAreaSyncAddon.ts:15-38]().

### Chat and AI Accessibility
Chat features provide specialized accessible views for inspecting code blocks and different help providers for panel chat, quick chat, edits view, and agent sessions [src/vs/workbench/contrib/chat/browser/actions/chatAccessibilityHelp.ts:28-76]().
*   **Context Keys**: The system uses context keys like `ChatContextKeys.inChatInput` to determine which help provider to activate [src/vs/workbench/contrib/chat/browser/actions/chatAccessibilityHelp.ts:62-62]().
...
graph LR
    ...
    D --> D1
```
**Sources:** [src/vs/platform/accessibilitySignal/browser/accessibilitySignalService.ts:73-73](), [src/vs/workbench/contrib/accessibility/browser/accessibleView.ts:71-71](), [src/vs/workbench/contrib/terminalContrib/accessibility/browser/textAreaSyncAddon.ts:15-15](), [src/vs/workbench/contrib/chat/browser/actions/chatAccessibilityHelp.ts:28-28]()

## Configuration and Settings

Accessibility behavior is governed by a set of verbosity and signal settings defined in `accessibilityConfiguration.ts` [src/vs/workbench/contrib/accessibility/browser/accessibilityConfiguration.ts:49-77]().

| Setting ID | Purpose | Source |
| :--- | :--- | :--- |
| `accessibility.verbosity.terminal` | Controls help hints when focusing the terminal | [src/vs/workbench/contrib/accessibility/browser/accessibilityConfiguration.ts:137-140]() |
| `accessibility.verbosity.editor` | Controls help hints for the code editor | [src/vs/workbench/contrib/accessibility/browser/accessibilityConfiguration.ts:60-60]() |
| `accessibility.signals.sound` | Global toggle for audio feedback (auto/on/off) | [src/vs/workbench/contrib/accessibility/browser/accessibilityConfiguration.ts:91-101]() |
| `editor.accessibilitySupport` | Main toggle for screen reader optimization | [src/vs/workbench/contrib/codeEditor/browser/accessibility/accessibility.ts:49-49]() |

**Sources:** [src/vs/workbench/contrib/accessibility/browser/accessibilityConfiguration.ts:49-140](), [src/vs/workbench/contrib/codeEditor/browser/accessibility/accessibility.ts:45-51]()
...
**Sources:** [src/vs/workbench/contrib/accessibility/browser/accessibleView.ts](), [src/vs/platform/accessibilitySignal/browser/accessibilitySignalService.ts](), [src/vs/workbench/contrib/terminalContrib/accessibility/browser/terminalAccessibleBufferProvider.ts]()